...
import { evaluateStoredConversationMatch } from "@/lib/messaging/matching-service";
...
const providers = [
  "GMAIL",
  ...(process.env.NODE_ENV !== "production" ? ["FAKE" as const] : []),
];
...
const valid = <T extends readonly string[]>(
  value: string | undefined,
  choices: T,
) =>
  value && choices.includes(value as T[number])
    ? (value as T[number])
    : undefined;
...
export default async function InboxPage({
  ...
  const filters: InboxFilters = {
    ...
    reviewState: valid(one(params.review), reviews) as
      | ConversationReviewState
      | undefined,
    classification: valid(one(params.classification), classifications) as
      | ConversationClassification
      | undefined,
    status: valid(one(params.status), statuses) as
      | ConversationStatus
      | undefined,
    provider: valid(one(params.provider), providers) as
      | MessageProvider
      | undefined,
    attachment: valid(one(params.attachment), [
      "attached",
      "unattached",
    ] as const),
    attention: parseInboxAttention(one(params.attention)),
    page:
      Number.isSafeInteger(pageValue) && pageValue > 0
        ? Math.min(pageValue, 10000)
        : 1,
  };
  ...
  const gmailJob = gmail
    ...
    : null;
  const gmailSummary = gmail?.lastImportSummary as {
  ...
  return (
    <div className="mx-auto max-w-375">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="flex items-center gap-3 text-3xl font-semibold tracking-[-0.04em]">
            <Inbox className="size-7" />
            Inbox
          </h1>
          <p className="mt-2 text-sm text-[#687080]">
            Review imported conversations and connect them to leads.
          </p>
        </div>
        {gmail ? (
          gmail.status === "CONNECTED" ? (
            ...
            <div className="text-right">
              <GmailConnectLink
                reconnect
                className="rounded-xl border border-amber-300 px-4 py-2.5 text-sm font-semibold text-amber-800 dark:text-amber-300"
              >
                Reconnect Gmail
              </GmailConnectLink>
              <p
                aria-live="polite"
                className="mt-2 max-w-md text-xs text-red-700 dark:text-red-300"
              >
                {gmail.lastSyncError ??
                  "Reconnect Gmail to resume synchronization."}
              </p>
            </div>
          )
        ) : (
          <GmailConnectLink className="rounded-xl bg-[#17181c] px-4 py-2.5 text-sm font-semibold text-white">
            Connect Gmail
          </GmailConnectLink>
        )}
      </header>

      <div className="inbox-shell mt-7 overflow-hidden rounded-2xl border border-black/[0.07] bg-white shadow-[0_8px_30px_rgba(23,24,28,0.035)] lg:grid lg:min-h-170 lg:grid-cols-[minmax(340px,420px)_minmax(0,1fr)]">
        <section
          aria-label="Conversation list"
          className={`${selectedId ? "hidden lg:block" : "block"} border-r border-black/[0.07]`}
        >
          <form
            action="/inbox"
            className="inbox-filters space-y-3 border-b border-black/[0.07] p-4"
          >
            <label className="block">
              ...
              <input
                ...
                placeholder="Search subject, sender, or lead…"
            ...
            <div className="grid grid-cols-2 gap-2">
              <Filter
                ...
                labelText="Attention"
                options={inboxAttentionValues}
                optionLabels={attentionLabels}
              />
              <Filter
                name="review"
              ...
              <Filter
                ...
                options={["attached", "unattached"]}
              />
              <button className="rounded-lg bg-[#17181c] px-3 py-2 text-sm font-semibold text-white">
                Apply
              </button>
            </div>
            {hasFilters && (
              <Link
                href="/inbox"
                className="inline-block text-xs font-semibold text-[#687080] underline"
              >
          ...
          <div>
            {list.items.map((conversation) => {
              ...
              return (
                <Link
                  key={conversation.id}
                  href={href(params, { conversation: conversation.id })}
                  aria-current={
                    selectedId === conversation.id ? "true" : undefined
                  }
                  className={`inbox-row block border-b border-black/6 p-4 focus-visible:outline-2 ${selectedId === conversation.id ? "inbox-row-selected bg-[#f0effb]" : "hover:bg-black/2.5"}`}
                >
                  ...
                  <p className="mt-1 truncate text-xs font-medium text-[#565d69]">
                    {conversation.latestMessage?.sender ?? "No participant"}
                  </p>
                  <p className="mt-1 line-clamp-2 text-xs leading-5 text-[#777e89]">
                    {conversation.latestMessage?.bodyPreview ??
                      "No message preview"}
                  </p>
                  <div className="mt-2 flex flex-wrap gap-1.5 text-[10px]">
                    <Badge text={label(conversation.provider)} />
                    <Badge text={label(conversation.classification)} />
                    <Badge text={label(conversation.reviewState)} />
            ...
            {!list.items.length && (
              <div className="p-10 text-center text-sm text-[#687080]">
                {filters.query
                  ? "No conversations match this search."
                  : hasFilters
                    ? "No conversations match these filters."
                    : gmail?.lastImportedAt
                      ? "No conversations have been imported."
                      : "Sync Gmail to import conversations."}
              </div>
            )}
          </div>
          <nav
        ...
        >
          {selectedId && !detail ? (
            <div className="grid min-h-125 place-items-center p-8 text-center">
              <div>
                <p className="font-semibold">Conversation not found</p>
                <p className="mt-2 text-sm text-[#687080]">
                  It may have been removed or is not accessible.
                </p>
                <Link
                  href={href(params, { conversation: undefined })}
            ...
            <ConversationDetail
              ...
              detail={detail}
              leads={leads}
              backHref={href(params, { conversation: undefined })}
  ...
  return (
    <label>
      ...
      <select
        name={name}
        defaultValue={value ?? ""}
        className="w-full rounded-lg border border-black/10 bg-white px-2 py-2 text-xs"
      >
...
function Badge({ text }: { text: string }) {
  return (
    <span className="rounded-md bg-[#f1f2f4] px-2 py-1 text-[#555c68]">
      {text}
    </span>
  );
}
...
function ConversationDetail({
  ...
  const company = companyPresentation(companyView, attachedLead?.company);
  return (
    <div>
      <header className="inbox-detail-header border-b border-black/[0.07] p-5 sm:p-6">
        <Link
          href={backHref}
          className="mb-4 inline-block text-sm font-semibold underline lg:hidden"
        >
          ← Back to inbox
        </Link>
        ...
        <div className="inbox-match mt-4 rounded-xl bg-[#f7f7f5] p-3 text-sm">
          <p>
            <span className="font-semibold">Lead name:</span>{" "}
            {attachedLead?.name ?? "None"}
          </p>
          <p className="mt-1">
            <span className="font-semibold">Company name:</span>{" "}
            {company.attachedCompany ?? "Not set"}
          </p>
          <p className="mt-1 text-[#687080]">
            <span className="font-semibold text-[#343840]">Match:</span>{" "}
            {matchPresentation.summary}
          </p>
        </div>
        {companyView && (
          <CompanySuggestion
      ...
      <div className="space-y-4 p-5 sm:p-6">
        {detail.messages.map((message) => {
          ...
          return (
            <article
              key={message.id}
              className={`inbox-message max-w-[85%] rounded-2xl p-4 ${message.direction === "OUTBOUND" ? "inbox-message-outbound ml-auto bg-[#ebe9fa]" : "inbox-message-inbound bg-[#f3f4f5]"}`}
            >
              <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
                <span className="font-semibold">{message.sender}</span>
                <time className="text-[#707783]">
                  {dateTime.format(message.receivedAt)}
                </time>
              </div>
